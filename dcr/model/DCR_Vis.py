...
from .model_utils import ClassifierWithLoss, ActionClassifierWithLoss_Vis
...
class DCR_Vis(nn.Module):
    def __init__(self, config, dataset):
        ...
        if config.classifier.action:
            self.action_classifier = ActionClassifierWithLoss_Vis(
        ...
        if not is_training:
            ...
            for i in range(1,1 + self.config.action_frame):
                ...
                if self.action_classifier is not None:
                    pred['action'], pred['uncertainty'] = self.action_classifier(out_frames[:, - i])
