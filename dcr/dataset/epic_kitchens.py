...
def add_gaussian_noise(input_tensor, mean=0, std=1, eta=0.1):
    
    noise = torch.normal(mean=mean, std=std, size=input_tensor.shape)
    noisy_input = input_tensor + noise * eta
...
class EPIC_KITCHENS_DATASET(torch.utils.data.Dataset):
    def __init__(self, logger, config):
        ...
        if config.name == 'EPIC-KITCHENS-55':
            ...
            with open(os.path.join('/data1/qzhb/datasets/EPIC55/EPIC55_action_statistic.pkl'), 'rb') as f:
                self.statistic_action_sim = pkl.load(f)

            with open(os.path.join('/data1/qzhb/datasets/EPIC55/EPIC55_action_conceptnet.pkl'), 'rb') as f:
            ...
            with open(os.path.join('/data1/qzhb/datasets/EPIC100/EPIC100_action_statistic.pkl'), 'rb') as f:
                self.statistic_action_sim = pkl.load(f)

            with open(os.path.join('/data1/qzhb/datasets/EPIC100/EPIC100_action_conceptnet.pkl'), 'rb') as f:
                self.conceptnet_action_sim = pkl.load(f)
    ...
    def __getitem__(self,i):
        ...
        if not self.challenge:
            ...
            label_sim = torch.tensor(list(self.conceptnet_action_sim[segment['next_action_class']].values()))
        ...
        if not self.add_noise:
            ...
            sample_feature = self._load_feat(
                segment['video_id'], 
                segment['frame_index'], 
            )
            out['past_frame'] = add_gaussian_noise(sample_feature, mean=0, std=1, eta=5)
