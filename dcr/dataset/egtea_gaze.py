...
class EGTEA_GAZE_DATASET(torch.utils.data.Dataset):
    def __init__(self, logger, config, root = None):
        ...
        temp = config.split
        if 'train' in config.split:
            split_num = temp.split('train')[1]
        elif 'valid' in config.split:
            split_num = temp.split('valid')[1]
        
        with open(os.path.join('/data1/qzhb/datasets/EGATEGazePlus/EGATEGazePlus_split' + str(split_num) + '_action_conceptnet.pkl'), 'rb') as f:
