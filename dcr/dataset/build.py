...
from .meccano import MECCANO_DATASET
...
def build_dataset(logger,config):
    ...
    elif config.name == 'MECCANO' :
        dataset = MECCANO_DATASET( logger, config)
...
def build_dataloader(logger, config, shuffle = False, ddp = False):
    ...
    if ddp:
        ...
        #### 控制训练集大小，resubmit时添加的实验
        adjust_dataset_size = False
        
        if not adjust_dataset_size:
            dataloader = DataLoader(
                dataset, batch_size=config.batch_size, num_workers=config.num_workers,
            ...
            dataloader = DataLoader(
                dataset, batch_size=config.batch_size, num_workers=config.num_workers,
                sampler=train_sampler, drop_last=False,collate_fn = collate_fn)
