...
class EPIC_KITCHENS100_Evaluator():
    def __init__(self,dataset ,root='./data/EK100'):
        ...
        ####  added high uncertainty classes for resubmit experiments
        ## high uncertainty class
        self.high_uncertainty_class_verbs = pd.read_csv('/data1/qzhb/datasets/EPIC100/EPIC100_high_uncertainty_verbs.csv', index_col="verb"
        ).index.values.tolist()
        self.high_uncertainty_class_nouns = pd.read_csv('/data1/qzhb/datasets/EPIC100/EPIC100_high_uncertainty_nouns.csv', index_col="noun"
        ).index.values.tolist()
        self.high_uncertainty_class_actions = pd.read_csv('/data1/qzhb/datasets/EPIC100/EPIC100_high_uncertainty_actions.csv', index_col="action"
        ).index.values.tolist()
        
        ## tail and high uncertainty class
        self.uncertainty_and_tail_class_verbs = [temp for temp in self.high_uncertainty_class_verbs if temp in self.tail_class_verbs]
        self.uncertainty_and_tail_class_nouns = [temp for temp in self.high_uncertainty_class_nouns if temp in self.tail_class_nouns]
        self.uncertainty_and_tail_class_actions = [temp for temp in self.high_uncertainty_class_actions if temp in self.tail_class_action]
    ...
    def __call__(self, prediction,gt):
        ...
        high_uncertainty_verb = self.compute_recall(rank_verb, gt['verb'], self.intersection(self.high_uncertainty_class_verbs,gt['verb']))
        high_uncertainty_noun = self.compute_recall(rank_noun, gt['noun'], self.intersection(self.high_uncertainty_class_nouns,gt['noun']))
        high_uncertainty_action = self.compute_recall(rank_action, gt['action'],self.intersection(self.high_uncertainty_class_actions,gt['action']))

        uncertainty_and_tail_verb = self.compute_recall(rank_verb, gt['verb'], self.intersection(self.uncertainty_and_tail_class_verbs,gt['verb']))
        uncertainty_and_tail_noun = self.compute_recall(rank_noun, gt['noun'], self.intersection(self.uncertainty_and_tail_class_nouns,gt['noun']))
        uncertainty_and_tail_action = self.compute_recall(rank_action, gt['action'],self.intersection(self.uncertainty_and_tail_class_actions,gt['action']))
        ...
        return OrderedDict([
            ...
            ('Uns_N',               unseen_noun),
            ('Uns_A',               unseen_action),
            ('HiUn_V',               high_uncertainty_verb),
            ('HiUn_N',               high_uncertainty_noun),
            ('HiUn_A',               high_uncertainty_action),
            ('TaHiUn_V',               uncertainty_and_tail_verb),
            ('TaHiUn_N',               uncertainty_and_tail_noun),
            ('TaHiUn_A',               uncertainty_and_tail_action)
        ])
