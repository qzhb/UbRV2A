...
def train(config, epoch, loader, model, optimizer, scheduler, bank, logger, writer):
    ...
    for i, batch in enumerate(loader):
        ...
        feedbatch = {
            ...
            for k,v in batch.items() if ('class' in k or 'frame' in k or 'action_label' in k) and ('frame_index' not in k) 
        }
        feedbatch.update({
...
def eval(config, epoch, loader, model, logger, evaluator):
    ...
    for i, batch in enumerate(loader):
        ...
        subbatch = {
            k : v.cuda(non_blocking=True)
            for k,v in batch.items() if ('class' in k or 'frame' in k) and ('frame_index' not in k) 
        }
...
def main(config):
    ...
    out_path = os.path.join(os.getcwd(), config.folder, config.name, current_time)
    ...
    for epoch in range(1 + init_epoch, config.train.max_epoch + 1):
        ...
        if epoch % config.eval.freq == 0:
            ...
            if result[config.main_metric] > best:
                #if config.train.data.name  == 'EPIC-KITCHENS-100':
